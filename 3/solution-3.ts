import { input, error } from "../utils.ts"
import { partition, maxBy, minBy } from "../deps.ts"
...
const gamma = parsed
    .reduce((acc, cur) =>
        acc.map((num, index) => num + cur[index])
    )
    .map(count =>
...
function calcLifeSupportRating(numbers: number[][], defaultBit: 0 | 1, index = 0): number[] {
    const choosePartition = defaultBit === 0 ? minBy : maxBy
    const partitions = partition(numbers, (it) => it[index] === defaultBit)
    const chosen = choosePartition(partitions, it => it.length) ?? error("Input emmpty")
